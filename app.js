...
class CricketScheduler {
    constructor() {
        this.baseURL = 'https://cricket-matches-scheduler.onrender.com';
    ...
    async handlePostAvailability(e) {
        ...
        try {
            const response = await fetch(`${this.baseURL}/api/availability/create`, {
                method: 'POST',
                headers: {
    ...
    async handleMatchAction(matchId, action) {
        try {
            const response = await fetch(`${this.baseURL}/api/match/confirm`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${this.token}`
    ...
    async loadUserTeam() {
        try {
            const response = await fetch(`${this.baseURL}/api/user/team`, {
    ...
    async loadOpenRequests() {
        try {
            ...
            const response = await fetch(`${this.baseURL}/api/availability/open?ground_type=free`);
    ...
    async loadPaidOpenRequests() {
        try {
            const response = await fetch(`${this.baseURL}/api/availability/open?ground_type=paid`);
    ...
    async loadChatMessages() {
        ...
        try {
            const response = await fetch(`${this.baseURL}/api/chat/${this.currentMatchId}`, {
                headers: { 'Authorization': `Bearer ${this.token}` }
            });
...
// Initialize app
>>>>>>> b0bdce782a66892031e5856da12ed145a37f7a05
